...
from yellowdog_client.model import ComputeRequirementTemplate
...
class ComputeServiceProxy:
    ...
    def update_compute_requirement(self, compute_requirement):
        return self._proxy.put(ComputeRequirement, compute_requirement, "requirements")
    ...
    def transition_compute_requirement(self, compute_requirement, next_status):
        url = "requirements/%s/transition/%s" % (compute_requirement.id, next_status.value)
    ...
    def stream_compute_requirement_updates(self, requirement: ComputeRequirement) -> EventSource:
    ...
    def is_compute_requirement_updating(self, compute_requirement: ComputeRequirement) -> bool:
        return self._proxy.raw_execute("GET", "requirements/%s/updating" % compute_requirement.id).json()
    ...
    def delete_compute_source_template(self, compute_source_template: ComputeSourceTemplate) -> None:
        self._proxy.delete("templates/sources/%s" % compute_source_template.id)
    ...
    def delete_compute_requirement_template(self, template: ComputeRequirementTemplate) -> None:
        self._proxy.delete("templates/requirements/%s" % template.id)
